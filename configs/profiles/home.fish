if test -f $NIX_DOTFILES/profiles/common.fish
    source $NIX_DOTFILES/profiles/common.fish
else
    echo "Error: common.fish not found in $NIX_DOTFILES/profiles/"
end

# Do stuff
abbr -a src "source $NIX_DOTFILES/profiles/home.fish"
abbr -a wp "nvim $NIX_DOTFILES/profiles/home.fish"

# cd into stuff
abbr -a wos "cd ~/Projects/web-os/"
abbr -a boot "cd ~/Projects/web-os/src/core/boot"
abbr -a ipc "cd ~/Projects/web-os/src/core/ipc"
abbr -a vfsapi "cd ~/Projects/web-os/src/core/vfs"
abbr -a rt "cd ~/Projects/web-os/src/runtime/"
abbr -a vfs "cd ~/Projects/web-os/server/vfs/"
abbr -a shell "cd ~/Projects/web-os/server/vfs/bin/shell"
abbr -a term "cd ~/Projects/web-os/server/vfs/apps/terminal"
abbr -a bin "cd ~/Projects/web-os/server/vfs/bin/"
abbr -a server "cd ~/Projects/web-os/server/"
abbr -a notes "cd ~/notes"
abbr -a bge "cd ~/Projects/bgevko-com/"
abbr -a @wos "cd ~/Projects/@web-os/"
abbr -a @shared "cd ~/Projects/@web-os/shared/"
abbr -a @cutils "cd ~/Projects/@web-os/color-utils/"
abbr -a @putils "cd ~/Projects/@web-os/path-utils/"
abbr -a @mc "cd ~/Projects/@web-os/message-channels/"
abbr -a @logger "cd ~/Projects/@web-os/logger/"
abbr -a @types "cd ~/Projects/@web-os/types/"
abbr -a ones "cd ~/Projects/oNes/"
abbr -a ng "cd ~/Projects/nes-game/"
abbr -a clsp "cd ~/.local/opt/ca65-lsp/"
abbr -a opt "cd ~/.local/opt"
abbr -a nt "cd ~/Projects/nes-tile/"
abbr -a xel "cd ~/.local/opt/xel"
abbr -a xels "cd ~/.local/opt/xel-solid"
abbr -a bpm "cd ~/Projects/bpm"
